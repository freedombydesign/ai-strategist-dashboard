...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/export-manager?error=asana_oauth_error`)
  }

  if (!code) {
    console.error('[Asana Callback] No code received')
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/export-manager?error=asana_no_code`)
  }
  ...
  try {
    ...
    const tokenResponse = await fetch('https://app.asana.com/-/oauth_token', {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: `${process.env.NEXTAUTH_URL}/api/auth/asana/callback`,
        code: code,
      }),
    })
    ...
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/export-manager?error=asana_callback_failed`)
  }
}