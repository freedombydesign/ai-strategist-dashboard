...
-- For now, allow all operations (will be restricted when user auth is added)
CREATE POLICY IF NOT EXISTS "Allow all operations on platform_connections" ON platform_connections
  FOR ALL USING (true);
...
CREATE TRIGGER IF NOT EXISTS trigger_update_platform_connections_updated_at
